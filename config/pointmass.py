...
class PtModel(nn.Module):
    ...
    def compute_decays(self):
        #TODO: check if this is reasonable/causing any errors
...
class PointmassMediumConfigModule:
    ENV_NAME = "MBRLPointmassMedium-v0"
    TASK_HORIZON = 150
    NTRAIN_ITERS = 15
    K_BINS = 100
    ...
    #TODO: Adapt these methods (and the optional above methods, if desired) to pointmass
    #TODO: add state density visualization code (not currently in repo)
    @staticmethod
    def obs_cost_fn(obs):
        # ee_pos = Poi._get_ee_pos(obs)
        #
    ...
    @staticmethod
    def ac_cost_fn(acs):
        _, rews, _, _ = [self.ENV.ac_rew(action) for action in acs]
        return rews
